...
from icecube.icetray.i3logging import log_error, log_info
from icecube.simprod import segments

from .recreate_mctree import RecreateMCTree
...
def RerunProposal(
    ...
    def prepare_repropagation(frame, mctree_name='I3MCTree'):
        ...
        needs_rerun = not (frame.Has(mctree_name) and
                           frame.Has('MMCTrackList'))
        ...
        if needs_rerun:
            ...
            if mctree_name + '_preMuonProp' + '_RNGState' in frame:
                ...
                log_info('Only found RNGState, make sure this is correct!')
            ...
            if meta_info['found_rng_state'] < 0.1 and fail_if_no_rng_found:
                msg = 'Could not find RNG state named {}!'
                raise ValueError(msg.format(rng_key))
    ...
    tray.AddModule(
        prepare_repropagation, 'prepare_repropagation'+name,
    ...
    tray.Add(segments.PropagateMuons, 'PropagateMuons'+name,
             RandomService=randomService,
             SaveState=True,
             InputMCTreeName=mctree_name + '_preMuonProp',
             OutputMCTreeName=mctree_name,
             If=lambda f: (not f.Has(mctree_name) or
                           not f.Has('MMCTrackList')))
    ...
    tray.AddModule(
        merge_background_trees, 'merge_background_trees'+name,
        mctree_name=mctree_name,
    ...
    tray.AddModule(
        recover_previous_keys, 'recover_previous_keys'+name,
        mctree_name=mctree_name,
        Streams=[icetray.I3Frame.DAQ],
    )
...
def AddMMCTrackList(tray, name):
    ...
    tray.AddModule(create_raw_mc_tree, 'create_raw_mc_tree',
                   Streams=[icetray.I3Frame.DAQ],
                   If=lambda f: (not f.Has('I3MCTree') or
                                 not f.Has('MMCTrackList')))

    tray.AddSegment(RecreateMCTree, 'RecreateMCTree',
                    MCTree="I3MCTree_new",
    ...
    tray.AddModule(rename_tree, 'rename_tree',
                   Streams=[icetray.I3Frame.DAQ])
    tray.AddModule('Delete', 'Delete RecreateMCTree',
                   Keys=['I3MCTreeRaw', 'I3MCTree_new'])
    # ---------------
