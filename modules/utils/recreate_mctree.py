...
def get_propagators():
    ...
    for pt in ['MuMinus', 'MuPlus', 'TauMinus', 'TauPlus']:
        propagators[getattr(dataclasses.I3Particle.ParticleType, pt)] = \
            mu_tau_prop
    for pt in ['DeltaE', 'Brems', 'PairProd', 'NuclInt', 'Hadrons', 'EMinus',
               'EPlus']:
...
def RecreateMCTree(tray, name, MCTree="I3MCTree", RawMCTree="I3MCTreeRaw",
    ...
    tray.AddModule('I3PropagatorModule', name+'Propagator',
    ...
    if Paranoia:
        ...
        tray.AddModule('I3PropagatorModule', name+'ParanoidPropagator',
                       PropagatorServices=Propagators,
                       RandomService=randomService,
                       RNGStateName=RNGState,
                       InputMCTreeName=RawMCTree,
        ...
        class MCTreeAudit(TestCase):
            ...
            def testParticleContent(self):
                ...
                for p1, p2 in zip(self.orig_tree, self.new_tree):
                    ...
                    self.assertClose(p1.energy, p2.energy,
                                     atol=1e-6, rtol=1e-9)
                    self.assertClose(p1.time, p2.time, atol=1, rtol=1e-9)
                    self.assertClose(p1.length, p2.length,
                                     atol=1e-6, rtol=1e-9)
                    for d in 'x', 'y', 'z':
        ...
        tray.AddModule('Delete', name+'DeleteParanoidCruft',
                       Keys=[ParanoidMCTree], If=audit)
