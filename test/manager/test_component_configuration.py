...
class TestConfiguration(unittest.TestCase):
    ...
    def test_object_initialization(self):
        ...
        for config_dict in config_dicts:
            ...
            self.assertEqual(
                configuration.mutable_settings, config_dict["mutable_settings"]
            )
            self.assertEqual(
                configuration.mutable_sub_components,
                config_dict["mutable_sub_components"],
            )
            self.assertEqual(
                configuration.check_values, config_dict["check_values"]
            )
            self.assertEqual(
                configuration.sub_component_configurations,
                config_dict["sub_component_configurations"],
            )
            self.assertEqual(
                configuration.dependent_sub_components,
                config_dict["dependent_sub_components"],
            )
            self.assertEqual(
                configuration.class_string, config_dict["class_string"]
            )
    ...
    def test_add_sub_components_key_error(self):
        ...
        with self.assertRaises(KeyError) as context:
            configuration.add_sub_components({"key_already_exists": component})
        self.assertTrue(
            "Sub component" in str(context.exception)
