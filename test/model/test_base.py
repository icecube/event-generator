...
class TestModel(unittest.TestCase):
    ...
    def test_method_assert_configured(self):
        ...
        self.assertTrue('Model is already set up!'
                        in str(context.exception))

        with self.assertRaises(TypeError) as context:
            model.assert_configured(None)
        self.assertTrue('Expected bool, but got'
                        in str(context.exception))
    ...
    def test_save_weights_variables_changed(self):
        ...
        with self.assertRaises(ValueError) as context:
            self.model.save_weights(directory)
        self.assertTrue('Model has changed since configuration call:'
                        in str(context.exception))
    ...
    def test_save_weights_file_already_exists(self):
        ...
        with self.assertRaises(IOError) as context:
            self.model.save_weights(directory)
        self.assertTrue('Checkpoint file ' in str(context.exception) and
    ...
    def test_save_weights_meta_data_already_exists(self):
        ...
        with self.assertRaises(KeyError) as context:
            self.model.save_weights(directory)
        self.assertTrue('Checkpoint index ' in str(context.exception) and
                        'already exists in meta dat' in str(context.exception))
    ...
    def test_save_weights_max_keep(self):
        ...
        num_models = len(glob.glob(os.path.join(directory,
                                                'model_checkpoint_00*.index')))
        self.assertEqual(3, len(glob.glob(os.path.join(
        ...
        self.model.save_weights(directory)
        self.assertEqual(0, len(glob.glob(os.path.join(
                            directory, 'model_checkpoint_00*05.index'))))
        self.assertEqual(3, len(glob.glob(os.path.join(
                            directory, 'model_checkpoint_00*.index'))))
        ...
        self.assertEqual(1, len(glob.glob(os.path.join(
                            directory, 'model_checkpoint_00*06.index'))))
        self.assertEqual(8, len(glob.glob(os.path.join(
                            directory, 'model_checkpoint_00*.index'))))
        ...
        self.assertEqual(0, len(glob.glob(os.path.join(
                            directory, 'model_checkpoint_00*08.index'))))
        self.assertEqual(1, len(glob.glob(os.path.join(
                            directory, 'model_checkpoint_00*09.index'))))
        self.assertEqual(1, len(glob.glob(os.path.join(
                            directory, 'model_checkpoint_00*013.index'))))
        self.assertEqual(0, len(glob.glob(os.path.join(
                            directory, 'model_checkpoint_00*014.index'))))
        self.assertEqual(1, len(glob.glob(os.path.join(
                            directory, 'model_checkpoint_00*016.index'))))
        self.assertEqual(8, len(glob.glob(os.path.join(
                            directory, 'model_checkpoint_00*.index'))))
    ...
    def test_method_load_weights_tracked_components_changed(self):
        ...
        with self.assertRaises(ValueError) as context:
            model.load(directory)
        self.assertTrue('Tracked components were changed!'
                        in str(context.exception))
    ...
    def test_method_load_weights_file_error(self):
        ...
        with self.assertRaises(IOError) as context:
            self.model.load_weights(directory)
        self.assertTrue('Could not find checkpoints meta data'
                        in str(context.exception))
