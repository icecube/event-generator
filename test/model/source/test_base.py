...
class TestSourceBase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.parameter_names = ['x', 'y', 'z', 'zenith', 'azimuth',
                                'energy', 'time']

        config = {'dummysourcesetting': 1337}
        self.source = self.get_dummy_source(config=config,
    ...
    def test_correct_configuration(self):
        ...
        self.assertTrue(isinstance(self.source.checkpoint,
                                   tf.train.Checkpoint))
        ...
        self.assertTrue(self.source.configuration.is_compatible(
                                                        self.configuration))
        ...
        self.assertDictEqual(
            ...
            {'name': 'egenerator.model.source.base',
             ...
             'parameter_index_dict':
                self.source._untracked_data['parameter_index_dict'],
             'parameter_name_dict':
                self.source._untracked_data['parameter_name_dict'],
             'parameter_names':
                ['x', 'y', 'z', 'zenith', 'azimuth', 'energy', 'time'],
             })
        self.assertEqual(self.source._sub_components,
                         {'data_trafo': self.data_trafo})
    ...
    def test_save_weights_meta_data(self):
        ...
        # Save model for the first time
        self.source.save_weights(directory, protected=True,
    ...
    def test_method_load(self):
        ...
        self.assertTrue(
            self.source._untracked_data['dummy_var'].numpy() == 42)
