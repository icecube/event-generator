...
class TestModuleDataHandler(unittest.TestCase):
    ...
    def test_method_load_modules(self):
        ...
        self.assertTrue(
            isinstance(sub_components["data_module"], PulseDataModule)
        )
        self.assertTrue(
            isinstance(
        ...
        self.assertTrue(
            isinstance(sub_components["weight_module"], DummyWeightModule)
        )
        self.assertTrue(
            isinstance(sub_components["misc_module"], DummyMiscModule)
        )
        self.assertTrue(
            isinstance(sub_components["filter_module"], DummyFilterModule)
        )
        ...
        self.assertTrue(
            "Modules have already been loaded!" in str(context.exception)
        )
    ...
    def test_methods_load_and_save(self):
        ...
        self.assertDictEqual(
            data_handler_new.configuration.dict,
...
class TestModuleDataHandlerOnTestData(unittest.TestCase):
    ...
    def test_get_data_from_hdf(self):
        ...
        self.assertListEqual(list(data[3][618:628, 1]), self.times_618_to_627)

        # check specific values for pulse charges
        self.assertListEqual(
            list(data[3][618:628, 0]), self.charges_618_to_627
        )
    ...
    def test_get_data_from_hdf_missing_label_key(self):
        ...
        """
        # check error message when attempting to overwrite file
        num_events, data = self.data_handler2.get_data_from_hdf(self.file_path)
    ...
    def test_get_batch_generator_forever(self):
        ...
        for i in range(10):
            ...
            self.assertTrue(
                (self.cascades_true[0] == data_batch[param_index][0]).all()
                or (self.cascades_true[1] == data_batch[param_index][0]).all()
            )
