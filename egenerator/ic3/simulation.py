...
class EventGeneratorSimulation(icetray.I3ConditionalModule):
    ...
    def __init__(self, context):
        ...
        self.AddParameter(
            ...
            "Particles below this energy threshold are not " "simulated.",
            100,
        )
        self.AddParameter(
        ...
        self.AddParameter(
            "SimulateElectronDaughterParticles",
    ...
    def Configure(self):
        ...
        self.max_simulation_distance = self.GetParameter(
            "max_simulation_distance"
        )
    ...
    def DAQ(self, frame):
        ...
        if self.store_pulse_prediction:
            ...
            dom_charges_total = np.sum(
                result_tensors["dom_charges"].numpy(), axis=0
            )  # sum over cascades
            ...
            for string in range(86):
                for om in range(60):
                    ...
                    pulse_series_map_pred[
                        icetray.OMKey(string + 1, om + 1)
                    ] = pulse_series
            ...
            for om in range(60):
                ...
                if self.merge_pulses:
                    if len(pulse_times) > 1:
                        time_bins = np.arange(
                            pulse_times[0],
                            pulse_times[-1] + self.pulse_merge_window,
                            self.pulse_merge_window,
                        )  # ns
                        n, bin_edges = np.histogram(
                            pulse_times, time_bins, weights=pulse_charges
                        )

                        pulse_times = bin_edges[
                            :-1
                        ]  # take left bin edge as time
                        pulse_charges = n
    ...
    def _get_light_sources(self, mc_tree, parent):
        ...
        if (
            parent.energy < self.min_simulation_energy
            or parent.pos.magnitude - parent.length
            > self.max_simulation_distance
        ):
            return [], []

        if parent.type in self.em_cascades:
            if self.simulate_electron_daughters and len(
                mc_tree.get_daughters(parent)
            ):
