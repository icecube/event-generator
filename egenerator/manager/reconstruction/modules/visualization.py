...
class Visualize1DLikelihoodScan:
    ...
    def make_1d_llh_scans(
        ...
        for param_i, parameter in enumerate(
            ...
            values_trafo = np.linspace(
                truth_trafo[0, param_i] - diff,
                truth_trafo[0, param_i] + diff,
                1000,
            )
            llh_vals_true = []
            ...
            for value_trafo in values_trafo:
                ...
                param_tensor = self.manager.data_trafo.inverse_transform(
                    data=param_tensor_trafo, tensor_name=parameter_tensor_name
                )
                values.append(param_tensor[0, param_i])

            offset_truth = loss_function(
                parameters_trafo=truth_trafo, data_batch=data_batch
            ).numpy()
            offset_reco = loss_function(
                parameters_trafo=result_trafo, data_batch=data_batch
            ).numpy()
            ...
            fig, axes = plt.subplots(
                2, 1, sharex=True, sharey=False, figsize=(9, 8)
            )
            ...
            for name, covariance in zip(covariance_names, covariance_list):
                ...
                for i, ax in enumerate(axes):
                    ...
                    parabola = (
                        offset
                        - basis_functions.log_gauss(
                            values, mu=mu_list[i], sigma=sigma
                        )
                        + offsets[i]
                    )
            ...
            # plot landscapes
            axes[0].plot(
                values, llh_vals_true, label="LLh (Truth)", color="0.2"
            )
            axes[0].axhline(offset_truth, color="0.8", linestyle="--")
            axes[1].plot(
                values, llh_vals_reco, label="LLh (Reco)", color="0.2"
            )
            ...
            if False:
                axes[0].set_ylim(
                    min(offset_truth - 0.1, min(llh_vals_true)),
                    offset_truth + 10,
                )
                axes[1].set_ylim(
                    min(offset_reco - 0.1, min(llh_vals_reco)),
                    offset_reco + 10,
                )
