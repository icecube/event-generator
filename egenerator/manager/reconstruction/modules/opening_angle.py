...
class CircularizedAngularUncertainty:
    ...
    def __init__(
        ...
        if self.unc_loss_function is None:
            self.unc_loss_function = manager.get_parameter_loss_function(
                **function_settings
            )
    ...
    def execute(self, data_batch, results, **kwargs):
        ...
        **kwargs
        ...
        def bisection_step(low, high, target=0.95, ddof=5):
            ...
            values = ([center], zen, azi, unc_results, unc_losses, cdf_value)
            if cdf_value > target:
                high = center
            else:
                low = center
        ...
        delta_psi = self.rng.uniform(
            1, unc_upper_bound, size=self.num_fit_points
        )
        ...
        circular_unc_deg = self.get_circularized_estimate(
            delta_psi, delta_loss, x0=5.0, ddof=ddof
        )
        ...
        print(
            "Uncertainty estimation took: {:3.3f}s".format(
    ...
    def get_circularized_estimate(self, delta_psi, delta_loss, x0=5.0, ddof=5):
        ...
        def loss_cdf(sigma, delta_psi, cdf_values):
            loss = (
                cdf_values - basis_functions.rayleigh_cdf(delta_psi, sigma)
            ) ** 2
            return np.sum(loss)
