...
class DefaultCascadeModel(Source):
    ...
    def _build_architecture(self, config, name=None):
        ...
        num_inputs = 11 + num_add_labels + num_snowstorm_params + 1 # new opening angle
    ...
    def get_tensors(self, data_batch_dict, is_training,
        ...
        # calculate opening angle between pmt orientation and neutrino direction
        opening_angle_pmt = angles.get_angle(tf.stack([cascade_dir_x, cascade_dir_y, cascade_dir_z], axis=-1) ,
                                         pmt_orientations
                                         )
        opening_angle_pmt = tf.expand_dims(opening_angle_pmt, axis=-1)

        # Normalize
        #opening_angle_pmt_trafo = (opening_angle_pmt - 1.5707963) /  0.72842141 harcoded
        ...
        #distance -= 1601.630935 # harcoded
        #distance /= 704.64192927

        #opening_angle_traf = ((opening_angle - 1.53529511) / harcoded
        #                      (0.613278453))
