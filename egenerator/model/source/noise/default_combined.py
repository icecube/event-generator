...
class DefaultNoiseModel(Source):
    ...
    def _build_architecture(self, config, name=None):
        ...
        self._untracked_data['local_vars_icecube'] = new_weights(
            shape=[2],
            stddev=1e-5,
            float_precision=float_precision,
            name='noise_scaling_icecube',
        )
    ...
    def get_tensors(self, data_batch_dict, is_training,
        ...
        dom_noise_rates_2=optical_module_2.dom_noise_rates
        ...
        # same but for icecube
        pulses_2 = data_batch_dict['x_pulses_2']
        ...
        # shape: [n_batch, 2]
        time_window_2 = data_batch_dict['x_time_window_2']

        # shape: [n_batch]
        livetime = time_window[:, 1] - time_window[:, 0]

        # shape: [n_batch]
        livetime_2 = time_window_2[:, 1] - time_window_2[:, 0]
        ...
        # shape: [n_batch, 1, 1, 1]
        livetime_exp_2 = tf.reshape(
            time_window_2[:, 1] - time_window_2[:, 0], [-1, 1, 1, 1])
        ...
        dom_noise_rates_2 = tf.reshape(dom_noise_rates_2.astype(param_dtype_np),
            shape=[1, num_strings_2, doms_per_string_2*num_pmts_2, 1],
        )

        # shape: [n_batch, num_strings, doms_per_string*num_pmts, 1]
        dom_charges = dom_noise_rates * livetime_exp
        dom_charges_2 = dom_noise_rates_2 * livetime_exp_2

        # shape: [n_batch]
        dom_pdf_constant = 1. / livetime
        dom_pdf_constant_2 = 1. / (livetime_2 + 1e-7)
        ...
        local_vars_2 = tf.nn.elu(self._untracked_data['local_vars_icecube']) + 1.01
        local_vars = local_vars + local_vars_2 * 0 # avoid problems when no Icecube data
        ...
        mean_scaling_2 = tf.reshape(
            tf.nn.elu(local_vars_2[0]) + 1.01, [1, 1, 1, 1])
        ...
        dom_charges_alpha_2 = tf.reshape(
            tf.nn.elu(local_vars_2[1] - 5) + 1.000001, [1, 1, 1, 1])

        # scale dom charge and uncertainty by learned scaling
        dom_charges = dom_charges * mean_scaling
        dom_charges_2 = dom_charges_2 * mean_scaling_2
        ...
        dom_charges_variance_2 = (
            dom_charges_2 + dom_charges_alpha_2*dom_charges_2**2)
        dom_charges_unc_2 = tf.sqrt(dom_charges_variance_2)
        ...
        pulse_pdf_2 = tf.gather(dom_pdf_constant_2, indices=pulses_ids_2[:, 0])
        ...
        tensor_dict['dom_charges_ice'] = dom_charges_2
        tensor_dict['dom_charges_alpha_ice'] = dom_charges_alpha_2
        tensor_dict['dom_charges_unc_ice'] = dom_charges_unc_2
        tensor_dict['dom_charges_variance_ice'] = dom_charges_variance_2
        tensor_dict['pdf_constant_ice'] = dom_pdf_constant_2
        tensor_dict['pdf_time_window_ice'] = time_window_2
        tensor_dict['pulse_pdf_ice'] = pulse_pdf_2
        if time_exclusions_exist:
