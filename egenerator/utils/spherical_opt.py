...
def fill_from_cart(s_vector):
    ...
    """
    radii = np.sqrt(
        s_vector["x"] ** 2 + s_vector["y"] ** 2 + s_vector["z"] ** 2
    )
    ...
    s_vec_zero["az"] = 0.0
    s_vec_zero["zen"] = 0.0
    s_vec_zero["coszen"] = 1.0
    s_vec_zero["sinzen"] = 0.0
    s_vec_zero["cosaz"] = 1.0
    s_vec_zero["sinaz"] = 0.0
...
def reflect(old, centroid, new):
    ...
    new["z"] = (
        2 * ca * cz * sz * x + 2 * cz * sa * sz * y + z * (cz**2 - sz**2)
    )
...
def centroid(cart_coords, sph_coord, axis=0):
    ...
    for dim in ["x", "y", "z"]:
        centroid_sph[dim] = (
            np.sum(sph_coord[dim], axis=axis) / sph_coord.shape[axis]
        )
...
def angular_dist(p1, p2):  # theta1, theta2, phi1, phi2):
    ...
    return np.arccos(
        p1["coszen"] * p2["coszen"]
        + p1["sinzen"] * p2["sinzen"] * np.cos(p1["az"] - p2["az"])
    )


def find_replacements(
    old_list, new_list, sorted_old_inds=None, sorted_new_inds=None
):
...
def spherical_opt(
    ...
    if not method in ["Nelder-Mead", "CRS2"]:
        raise ValueError(
            "Unknown method %s, choices are Nelder-Mead or CRS2" % method
        )
    ...
    if method == "Nelder-Mead":
        assert (
            n_points == n_dim + 1
        ), "Nelder-Mead will need n+1 points for an n-dimensional function"

    if method == "CRS2":
        assert n_points > n_dim, "CRS will need more points than dimensions"
        assert (
            n_points - 1
        ) / n_dim >= batch_size, "{} must be >= batch size ({})!".format(
            (n_points - 1) / n_dim, batch_size
        )
    ...
    if cstdthresh is not None:
        assert (
            len(cstdthresh) == n_cart
        ), "Std-dev stopping values for Cartesian coordinates must have length equal to number of Cartesian coordinates"
        cstdthresh = np.array(cstdthresh)
        cstdthresh_gtz_mask = cstdthresh > 0
        if np.count_nonzero(cstdthresh_gtz_mask) == 0:
            cstdthresh = None
        elif meta:
            meta_dict["cstdthresh_met_at_iter"] = np.full(
                np.count_nonzero(cstdthresh_gtz_mask), -1
            )

    if sstdthresh is not None:
        assert (
            len(sstdthresh) == n_spher
        ), "Std-dev stopping values for spherical coordinates must have length equal to number of spherical coordinate pairs"
        sstdthresh = np.array(sstdthresh)
        sstdthresh_gtz_mask = sstdthresh > 0
        if np.count_nonzero(sstdthresh_gtz_mask) == 0:
            sstdthresh = None
        elif meta:
            meta_dict["sstdthresh_met_at_iter"] = np.full(
                np.count_nonzero(sstdthresh_gtz_mask), -1
            )
    ...
    all_cartesian_indices = list(
        set(range(n_dim)) ^ set(all_spherical_indices)
    )
    ...
    for iter_num in range(max_iter + 1):
        ...
        if cstdthresh is not None or sstdthresh is not None:
            if cstdthresh is not None:
                ...
                converged = (
                    cstd[cstdthresh_gtz_mask] < cstdthresh[cstdthresh_gtz_mask]
                )
                if meta:
                    mask = np.logical_and(
                        meta_dict["cstdthresh_met_at_iter"] < 0, converged
                    )
            ...
            if sstdthresh is not None:
                ...
                for sph_pair_idx, stdthresh in enumerate(sstdthresh):
                    if stdthresh > 0:
                        ...
                        std = np.sqrt(
                            np.sum(np.square(deltas)) / (n_points - 1)
                        )
                        sstd[sph_pair_idx] = std
                        converged = converged and std < stdthresh
                        if meta:
                            if (
                                meta_dict["sstdthresh_met_at_iter"][thresh_idx]
                                < 0
                                and std < stdthresh
                            ):
                                meta_dict["sstdthresh_met_at_iter"][
                                    thresh_idx
                                ] = iter_num
                        thresh_idx += 1
                    else:
        ...
        if method == "CRS2":
            ...
            batch_indices = rand.choice(
                n_points - 1, (batch_size, n_dim), replace=False
            )
            ...
            cart_pts = s_cart[centroid_indices.flat].reshape(
                (batch_size, n_dim, n_cart)
            )
            spher_pts = s_spher[centroid_indices.flat].reshape(
                (batch_size, n_dim, n_spher)
            )
            # shape is (batch_size, n_dim, n_cart/n_sph). We want to average along axis 1
            centroid_cart, centroid_spher = centroid(
                cart_pts, spher_pts, axis=1
            )

            # create reflected points
            reflected_p_carts = (
                2 * centroid_cart - s_cart[batch_indices[:, -1]]
            )
            reflected_p_sphers = np.zeros((batch_size, n_spher), dtype=SPHER_T)
            reflect(
                s_spher[batch_indices[:, -1]],
                centroid_spher,
                reflected_p_sphers,
            )
            ...
            replacements = find_replacements(
                fvals, new_fvals, sorted_idx, sorted_new
            )
            ...
            inds_to_replace = sorted_idx[
                : len(sorted_idx) - n_simplex_replaces
            ]
            ...
            for i, dim in enumerate(["x", "y", "z"]):
                w_i = w[i]
                mutated_p_sphers[dim] = (1 - w_i) * s_spher[best_idx][
                    dim
                ] + w_i * help_p_spher[dim]
            ...
            centroid_cart, centroid_spher = centroid(
                s_cart[centroid_indices], s_spher[centroid_indices]
            )
            ...
            if (
                reflected_fval < fvals[sorted_idx[-2]]
                and reflected_fval >= fvals[best_idx]
            ):
                # found better point
            ...
            if reflected_fval < fvals[best_idx]:
                ...
                expanded_p_cart = 2.0 * reflected_p_cart - centroid_cart
            ...
            if reflected_fval < fvals[worst_idx]:
                ...
                contracted_p_cart, contracted_p_spher = centroid(
                    np.vstack([centroid_cart, reflected_p_cart]),
                    np.vstack([centroid_spher, reflected_p_spher]),
                )
                ...
                contracted_p_cart, contracted_p_spher = centroid(
                    np.vstack([centroid_cart, s_cart[worst_idx]]),
                    np.vstack([centroid_spher, s_spher[worst_idx]]),
                )
                contracted_p = create_x(contracted_p_cart, contracted_p_spher)
                contracted_fval = func(contracted_p)
                n_calls += 1
            ...
            for idx in range(n_points):
                if not idx == best_idx:
                    s_cart[idx], s_spher[idx] = centroid(
                        s_cart[[best_idx, idx]], s_spher[[best_idx, idx]]
                    )
