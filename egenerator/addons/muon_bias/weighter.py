...
class BiasedMuonWeighter(icetray.I3ConditionalModule):
    ...
    def __init__(self, context):
        ...
        self.AddParameter('model_name',
                          ...
                          'model path.')
        self.AddParameter('bias_function',
        ...
        # Optional settings
        self.AddParameter('keep_all_events',
        ...
        self.AddParameter('outer_veto_hull',
        ...
        self.AddParameter('inner_veto_hull',
        ...
        self.AddParameter('track_bin_size',
                          ...
                          30)
        self.AddParameter('bad_doms_key',
        ...
        self.AddParameter('output_key',
                          'The output base key to which bias weights will be '
                          'saved.',
                          'BiasedMuonWeighter')
        self.AddParameter('mc_tree_name',
                          ...
                          'I3MCTree')
        self.AddParameter('mmc_track_list_name',
                          'The name of the MMCTrackList which contains the '
                          'MuonGun primary muon.',
                          'MMCTrackList')
        self.AddParameter('save_debug_info',
        ...
        self.AddParameter('verbose',
                          'If True, more verbose output is provided.',
                          False)
    ...
    def Configure(self):
        ...
        included_parameters = [
            'azimuth', 'zenith', 'energy', 'time', 'x', 'y', 'z',
            'cascade_azimuth',
            'cascade_energy',
            'cascade_time',
            'cascade_x',
    ...
    def DAQ(self, frame):
        ...
        for parameter_tensor in parameter_tensors:
            if parameter_tensor is None:
                ...
                dom_charges = np.squeeze(np.sum(
                    result_tensors['dom_charges'], axis=0))
        ...
        for i, layer_distance in enumerate(layer_distances):
            if layer_distance is None:
                ...
                layer_energies.append(float(np.sum(
                    parameter_tensors[i][:, self.energy_index])))
        ...
        keep_prob = self.bias_function(bias_data)
    ...
    def get_muongun_track(self, frame, muon):
        ...
        """
        for track in MuonGun.Track.harvest(frame[self.mc_tree_name],
                                           frame[self.mmc_track_list_name]):
            if track.id == muon.id:
                return track
        return None
    ...
    def get_muon_layer_distances(self, muon):
        ...
        assert not geometry.point_is_inside(self.icecube_hull, muon.pos), \
        ...
        if last_dist_entry is None:
            ...
            distance = max(0., mu_utils.get_distance_along_track_to_point(
                muon.pos, muon.dir, dataclasses.I3Position(0., 0., 0.)))
            layer_distances.append((0., distance))
        else:
            layer_distances.append((0., last_dist_entry))
    ...
    def get_parameter_tensors(self, layer_distances, track):
        ...
        for layer_distance in layer_distances:
            if layer_distance is None:
                ...
                n_bins = max(
                    int(np.diff(layer_distance) / self.track_bin_size),
                    1) + 1
