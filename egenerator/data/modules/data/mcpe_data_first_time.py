    ...
    def get_data_from_hdf(self, file, *args, **kwargs):
        ...
        **kwargs
        ...
        for pulse_index, row in enumerate(pulses.itertuples()):
            ...
            # pulse charge: row[11], time: row[10]
            # accumulate charge in DOMs
            x_dom_charge[index, string-1, (dom-1)*num_pmts + pmt, 0] += row[11]

            key = (index, string-1, (dom-1)*num_pmts + pmt)

            if key not in pulse_data:
                pulse_data[key] = [row[11], row[10]]
            else:
                pulse_data[key][0] += row[11] 
                pulse_data[key][1] = min(pulse_data[key][1],row[10])
            
            if key not in pulse_data_index:
                pulse_data_index[key] = [index, string-1, (dom-1)*num_pmts + pmt]
        ...
        x_pulses = np.array(list(pulse_data.values()),
                            dtype=self.data['np_float_precision'])
        x_pulses_ids = np.array(list(pulse_data_index.values()),dtype=np.int32)
        ...
        **kwargs
        ...
        for omkey, pulse_list in pulses.items():
            ...
            for pulse in pulse_list:
                ...
                # pulse npe: row[11], time: row[10]
                # accumulate charge in DOMs
                x_dom_charge[index, string-1, (dom-1)*num_pmts + pmt, 0] += pulse.npe

                key = (string-1, (dom-1)*num_pmts + pmt)

                if key not in pulse_data:
                    pulse_data[key] = [pulse.npe, pulse.time]
                else:
                    pulse_data[key][0] += pulse.npe
                    pulse_data[key][1] = min(pulse_data[key][1],pulse.time)
        ...
        x_pulses = np.array(list(pulse_data.values()), dtype=self.data['np_float_precision'])
        x_pulses_ids = np.array(list(pulse_data_index.values()), dtype=np.int32)
