...
class PulseDataModule(BaseComponent):
    ...
    """Pulse data module for combined detectors

    This is an initial version to combine the data extraction of different detector layouts
    mainly IceCube and IceCube-Gen2
    ...
    def _configure(self, config_data, pulse_key_det1, pulse_key_det2, dom_exclusions_key,
                   time_exclusions_key, float_precision, add_charge_quantiles,
                   discard_pulses_from_excluded_doms, optical_module_key1, optical_module_key2):
        ...
        ValueError
            Description
        """
        if not isinstance(pulse_key_det1, str):
            msg = 'Pulse key type: {!r} != str'
            raise TypeError(msg.format(type(pulse_key_det1)))
        ...
        optical_module = DetectorInfoModule(optical_module_key1)
        ...
        optical_module_2 = DetectorInfoModule(optical_module_key2)
        self.optical_module_2 = optical_module
        num_strings_2=optical_module_2.num_strings
        doms_per_string_2=optical_module_2.doms_per_string
        num_pmts_2 = optical_module_2.num_pmts #Number of pmts per module
        ...
        x_dom_charge = DataTensor(name='x_dom_charge',
                                  ...
                                  trafo_log=True,
                                  trafo_batch_axis=0)
        x_dom_charge_2 = DataTensor(name='x_dom_charge_2',
                                  shape=[None, num_strings_2, doms_per_string_2 * num_pmts_2, 1],
                                  tensor_type='data',
                                  dtype=float_precision,
                                  trafo=True,
                                  trafo_reduce_axes=(1, 2),
        ...
        x_pulses_ids = DataTensor(name='x_pulses_ids',
                                  ...
                                  vector_info={'type': 'index',
                                               'reference': 'x_pulses'},
                                  dtype='int32')
        x_pulses_2 = DataTensor(name='x_pulses_2',
                              shape=[None, pulse_dim],
                              tensor_type='data',
                              vector_info={'type': 'value',
                                           'reference': 'x_pulses_ids_2'},
                              dtype=float_precision)
        x_pulses_ids_2 = DataTensor(name='x_pulses_ids_2',
                                  shape=[None, 3],
                                  tensor_type='data',
                                  vector_info={'type': 'index',
                                               'reference': 'x_pulses_2'},
                                  dtype='int32')
        x_time_window = DataTensor(name='x_time_window',
                                   shape=[None, 2],
                                   tensor_type='data',
                                   dtype=float_precision)
        x_time_window_2 = DataTensor(name='x_time_window_2',
                                   shape=[None, 2],
                                   tensor_type='data',
                                   dtype=float_precision)
        ...
        data['data_tensors'] = DataTensorList([
            ...
            x_time_window, x_time_exclusions, x_time_exclusions_ids, x_dom_charge_2, x_pulses_2, x_pulses_ids_2, x_time_window_2])
        ...
        configuration = Configuration(
            ...
            mutable_settings=dict(
                optical_module_key1=optical_module_key1,
                optical_module_key2=optical_module_key2,
                pulse_key_det1=pulse_key_det1,
                pulse_key_det2=pulse_key_det2,
    ...
    def get_data_from_hdf(self, file, *args, **kwargs):
        ...
        **kwargs
        ...
        try:
            pulses = f[self.configuration.config['pulse_key_det1']]
            pulses_2 = f[self.configuration.config['pulse_key_det2']]
            _labels = f['LabelsDeepLearning']

            optical_module = DetectorInfoModule(self.configuration.config['optical_module_key1'])
            optical_module_2 = DetectorInfoModule(self.configuration.config['optical_module_key2'])
        ...
        bad_doms_mask_2=optical_module_2.bad_doms_mask
        

        x_dom_charge = np.zeros([size, num_strings, doms_per_string * num_pmts, 1],
                                dtype=self.data['np_float_precision'])
        
        x_dom_charge_2 = np.zeros([size, num_strings_2, doms_per_string_2 * num_pmts_2, 1],
                                dtype=self.data['np_float_precision'])
        if self.data['dom_exclusions_exist']:
        ...
        num_pulses_1 = len(pulses['Event'])
        num_pulses_2 = len(pulses_2['Event'])

        x_pulses = np.empty((num_pulses_1, pulse_dim),
                            dtype=self.data['np_float_precision'])
        x_pulses_ids = np.empty((num_pulses_1, 3), dtype=np.int32) #modifiy

        x_pulses_2 = np.empty((num_pulses_2, pulse_dim),
                            dtype=self.data['np_float_precision'])
        x_pulses_ids_2 = np.empty((num_pulses_2, 3), dtype=np.int32) #modifiy
        ...
        x_time_window_2 = np.empty(
            [size, 2],  dtype=self.data['np_float_precision'])
        x_time_window_2[:, 0] = float('inf')
        x_time_window_2[:, 1] = -float('inf')

        # ---------------------
        # get pulse information
        # ---------------------
        for pulse_index, row in enumerate(pulses_2.itertuples()):
            string = row[6] 
            dom = row[7]
            pmt = row[8]
            assert pmt==0, "IceCube PMTs should always be 0."
            if dom > doms_per_string_2 or pmt>=num_pmts_2:
            ...
            if index<0:
                print("En los pulsos Icecube",index,string,dom,pmt,event_dict)

            # pulse charge: row[12], time: row[10]
            # accumulate charge in DOMs
            x_dom_charge_2[index, string-1, (dom-1)*num_pmts_2 + pmt, 0] += row[12]

            # gather pulses
            if add_charge_quantiles:

                # (charge, time, quantile)
                cum_charge = float(x_dom_charge_2[index, string-1, (dom-1)*num_pmts_2 + pmt, 0])
                x_pulses_2[pulse_index] = [row[12], row[10], cum_charge]

            else:
                # (charge, time)
                x_pulses_2[pulse_index] = [row[12], row[10]]

            # gather pulse ids (batch index, string, dom)
            x_pulses_ids_2[pulse_index] = [index, string-1, dom-1]

            # update time window
            if row[10] > x_time_window_2[index, 1]:
                x_time_window_2[index, 1] = row[10]
            if row[10] < x_time_window_2[index, 0]:
                x_time_window_2[index, 0] = row[10]
        ...
        for pulse_index, row in enumerate(pulses.itertuples()):
            ...
            index = event_dict[(row[1:5])]
            if index<0:
                tf.print("En los pulsos normales",index,string,dom,pmt)
            # pulse charge: row[12], time: row[10]
            # accumulate charge in DOMs
        ...
        data_dict = {
            ...
            'x_dom_charge_2': x_dom_charge_2,
            'x_dom_exclusions': x_dom_exclusions,
            'x_pulses': x_pulses,
            'x_pulses_2': x_pulses_2,
            'x_pulses_ids': x_pulses_ids,
            'x_pulses_ids_2': x_pulses_ids_2,
            'x_time_window': x_time_window,
            'x_time_window_2': x_time_window_2,
    ...
    def get_data_from_frame(self, frame, *args, **kwargs):
        ...
        **kwargs
        ...
        # get pulses
        pulses = frame[self.configuration.config['pulse_key_det1']]
        pulses_2 = frame[self.configuration.config['pulse_key_det2']]
        ...
        for omkey, pulse_list in pulses_2.items():
            ...
            for pulse in pulse_list:
                ...
                # pulse charge: row[12], time: row[10]
                # accumulate charge in DOMs
                x_dom_charge_2[index, string-1, dom-1, 0] += pulse.charge

                # gather pulses
                if add_charge_quantiles:

                    # (charge, time, quantile)
                    cum_charge = float(x_dom_charge_2[index, string-1, dom-1, 0])
                    x_pulses_2.append([pulse.charge, pulse.time, cum_charge])

                else:
                    # (charge, time)
                    x_pulses_2.append([pulse.charge, pulse.time])

                # gather pulse ids (batch index, string, dom)
                x_pulses_ids_2.append([index, string-1, dom-1])

                # update time window
                if pulse.time > x_time_window_2[index, 1]:
                    x_time_window_2[index, 1] = pulse.time
                if pulse.time < x_time_window_2[index, 0]:
                    x_time_window_2[index, 0] = pulse.time

        x_pulses_2 = np.array(x_pulses_2, dtype=self.data['np_float_precision'])
        x_pulses_ids_2 = np.array(x_pulses_ids_2, dtype=np.int32)

        # convert cumulative charge to fraction of total charge, e.g. quantile
        if add_charge_quantiles:

            # compute flat indices
            dim1 = x_dom_charge_2.shape[1]
            dim2 = x_dom_charge_2.shape[2]
            flat_indices = (x_pulses_ids_2[:, 0]*dim1*dim2 +  # event
                            x_pulses_ids_2[:, 1]*dim2 +  # string
                            x_pulses_ids_2[:, 2]) # All PMTs of a string
            # flatten dom charges
            flat_charges_2 = x_dom_charge_2.flatten()

            # calculate quantiles
            x_pulses_2[:, 2] /= flat_charges_2[flat_indices]
