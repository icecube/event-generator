    ...
    def get_data_from_hdf(self, file, *args, **kwargs):
        ...
        **kwargs
        ...
        try:
            ...
            for l in ['track_anchor_x', 'track_anchor_y', 'track_anchor_z', 'zenith',
                      'azimuth', 'track_anchor_time']:
                track_parameters.append(_labels[l])

            # add dummy energy loss values
            track_energy = _labels['track_energy']

            num_cascades = self.configuration.config['num_cascades']
            energy_per_track = np.clip(track_energy / num_cascades,
                                         100, float('inf'))

            track_parameters.extend([energy_per_track] * num_track)
