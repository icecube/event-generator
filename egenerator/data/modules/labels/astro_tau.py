...
class AstroTauGeneratorLabelModule(BaseComponent):
    ...
    def __init__(self, logger=None):
        ...
        super(AstroTauGeneratorLabelModule, self).__init__(logger=logger)
    ...
    def _configure(self, config_data, shift_cascade_vertex, trafo_log,
        ...
        if additional_labels is None:
            additional_labels = []
        parameter_names = [
            'TauPositionX', 'TauPositionY', 'TauPositionZ', 'TauZenith',
            'TauAzimuth', 'Cascade1Energy', 'TauTime',
        ...
        data['label_tensors'] = DataTensorList([DataTensor(
            name='x_parameters',
            shape=[None, len(parameter_names)],
            tensor_type='label',
    ...
    def _shift_parameters(self, parameters):
        ...
        zenith = parameters[param_dict['TauZenith']]
        azimuth = parameters[param_dict['TauAzimuth']]
        ...
        shift = self._get_cascade_extension(
            parameters[param_dict['Cascade1Energy']])

        parameters[param_dict['TauPositionX']] += dir_x * shift
        parameters[param_dict['TauPositionY']] += dir_y * shift
        parameters[param_dict['TauPositionZ']] += dir_z * shift
        parameters[param_dict['TauTime']] += shift / c

        shift2 = self._get_cascade_extension(
            parameters[param_dict['Cascade1Energy']])

        # we need to compensate for the shift of the anchor point
        parameters[param_dict['TauLength']] -= shift

        parameters[param_dict['TauLength']] += shift2
